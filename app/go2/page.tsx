  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      if (scrollPercent >= 0.25) {
        ...
        trackMetaGo('ViewContent', { content_name: '30 in 30 Landing' });
...
function QuestionnaireModal({ onClose }: { onClose: () => void }) {
  ...
  const handleFinalSubmit = async () => {
    ...
    body.append('subject', `Roofing Lead (30in30): ${formData.name} — ${formData.city}`);
...
export default function RoofingLanding() {
  ...
  useEffect(() => {
    trackPageVisit('/go2').then((id) => { visitIdRef.current = id; });
          ...
          <div className="flex items-center justify-center gap-1 mb-1">
            {[...Array(5)].map((_, i) => (
              <div key={i} className="w-6 h-6 flex items-center justify-center" style={{ backgroundColor: '#00b67a' }}>
                <svg viewBox="0 0 24 24" width="14" height="14"><path d="M12 0l3.09 6.26L22 7.27l-5 4.87 1.18 6.88L12 15.4l-6.18 3.62L7 12.14 2 7.27l6.91-1.01L12 0z" fill="white"/><line x1="16.55" y1="12.15" x2="9" y2="16.8" stroke="#009E68" stroke-width="0.9"/></svg>
              </div>
            ))}
          </div>
                ...
                <div className="bg-white rounded-2xl shadow-md p-3">
                  <div className="rounded-xl overflow-hidden bg-black aspect-video">
                    <iframe
                      ...
                      allowFullScreen
                      title={`Success Story Video ${index + 2}`}
                    />
                  </div>
                </div>
              </div>
