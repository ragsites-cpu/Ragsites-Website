      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex items-center justify-center gap-4 mb-16">
          ...
          <button
            ...
            className={`relative w-14 h-7 rounded-full transition-colors duration-300 ${
              isYearly ? 'bg-cyber-cyan' : 'bg-white/20'
            }`}
          >
            <div
          ...
          <span className={`text-sm font-medium ${isYearly ? 'text-white' : 'text-gray-500'}`}>
            Yearly
            <span className="ml-2 text-xs text-cyber-cyan font-semibold">Save 20%</span>
          </span>
        </div>
          ...
          {plans.map((plan) => (
            <div
              ...
              className={`relative rounded-2xl p-8 flex flex-col transition-all duration-300 hover:-translate-y-1 ${
                plan.popular
                  ? 'glass-card border-2 border-cyber-cyan/50 shadow-[0_0_30px_rgba(0,240,255,0.15)]'
                  : 'glass-card'
              }`}
            >
              {plan.popular && (
                <div className="absolute -top-4 left-1/2 -translate-x-1/2 px-4 py-1 rounded-full bg-gradient-to-r from-cyber-cyan to-cyber-purple text-black text-xs font-bold uppercase tracking-wider">
                  Most Popular
                </div>
              )}
              ...
              <ul className="space-y-3 mb-8 flex-1">
                {plan.features.map((feature) => (
                  <li key={feature} className="flex items-start gap-3">
                    <Check className="w-5 h-5 text-cyber-cyan flex-shrink-0 mt-0.5" />
              ...
              <button
                ...
                className={`w-full py-3 rounded-full font-semibold text-sm transition-all duration-200 hover:scale-[1.02] ${
                  plan.popular
                    ? 'bg-gradient-to-r from-cyber-cyan to-cyber-purple text-black'
        ...
        <div className="mt-12 text-center glass-card p-8 rounded-2xl">
          ...
          <button
            ...
            className="px-8 py-3 rounded-full border border-cyber-cyan/50 text-cyber-cyan font-semibold text-sm hover:bg-cyber-cyan/10 transition-colors"
          >
            Contact Sales
          </button>
        </div>
